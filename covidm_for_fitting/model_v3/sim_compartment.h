...
public:
    ...
    // Execute one time step's events
    bool Tick(Parameters& P, Randomizer& Rand, double t, Reporter& rep);

    // Run the model
    void Run(Parameters& P, Randomizer& Rand, Reporter& rep, vector<double> x_fit = vector<double>());
    
    // Run the model until time t
    void RunUntil(Parameters& P, Randomizer& Rand, Reporter& rep, vector<double>& x_fit, double t_from, double t_to, bool include_to);
