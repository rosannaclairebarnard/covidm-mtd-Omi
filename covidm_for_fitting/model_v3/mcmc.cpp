...
void HandleException(std::exception& e, std::vector<string>& param_names, std::vector<double>& theta, unsigned int chain_number)
{
    std::string message = string("Caught error in target function in DEMCMC_Priors for chain ") + 
        to_string(chain_number) + string(". Writing theta to file error_log_CN.csv.\n");
    cout << message << flush;
    
    ofstream error_log("./error_log_" + to_string(chain_number) + ".csv");
    for (auto& n : param_names) {
        error_log << n << (n == param_names.back() ? "" : ", ");
    }
    error_log << "\n";
    error_log << std::setprecision(std::numeric_limits<double>::max_digits10);
    
    for (auto& x : theta) {
        error_log << x << (x == theta.back() ? "" : ", ");
    }
    error_log << "\n";
    error_log.close();
    
    string notify_command = 
                    string("curl -s --form-string \"token=arckmxd33fp57d3ze6dujxeg1in48s\" ") +
                    string("--form-string \"user=u3afjhjahuvoksvo5ybzv77mkdnzme\" ") +
                    string("--form-string \"message=Error caught. Stopping.\" https://api.pushover.net/1/messages.json");
    
    std::system(notify_command.c_str());
    ...
    vector<vector<double>> init, int init_iter)
{
    ...
    for (int c = 0; c < n_chains; ++c)
    {
        try {
            ...
            HandleException(e, param_names, chains[c], c);
        }
    }
    ...
    for (int i = init_iter; i < burn_in + iterations; ++i)
    {
        ...
        for (int c = 0; c < n_chains; ++c)
        {
            ...
            try {
                p_p = target(theta_p, l_p);
            } catch(std::exception& e) {
                HandleException(e, param_names, theta_p, c);
            }
