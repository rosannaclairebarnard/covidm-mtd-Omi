...
double Randomizer::TDist(double nu, double x0, double scale)
{
    return x0 + gsl_ran_tdist(r, nu) * scale;
}
...
void Randomizer::Sample(unsigned int N, std::vector<double>& p, std::vector<unsigned int>& i, bool stratified)
{
    i.resize(N, 0);
    ...
    if (!stratified)
    {
        ...
        Multinomial(N, p, draw);
        
        for (unsigned int k = 0, j = 0; k < draw.size(); ++k)
        {
            for (unsigned int m = 0; m < draw[k]; ++m, ++j)
            {
                i[j] = k;
            }
        }
    ...
    {
        double total_mass = std::accumulate(p.begin(), p.end(), 0.0);
        double strat_mass = total_mass / N;
        unsigned int index = 0;
        double bound = 0.0;
        for (unsigned int j = 0; j < N; ++j)
        {
            double dart = j * strat_mass + Uniform(0.0, strat_mass);
            while (bound + p[index] < dart)
            {
                bound += p[index];
                ++index;
                if (index >= p.size()) 
