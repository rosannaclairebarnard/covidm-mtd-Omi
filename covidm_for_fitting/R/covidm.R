...
cm_version_        = cm_option_("cm_version", 3)
...
} else if (cm_version_ == 3) {
    source(paste0(cm_path_, "/R/v3/cm3_backend.R"))
    source(paste0(cm_path_, "/R/v3/cm3_run.R"))
    source(paste0(cm_path_, "/R/v3/cm3_params.R"))
    source(paste0(cm_path_, "/R/v3/cm3_interventions.R"))
    source(paste0(cm_path_, "/R/v3/cm3_fit.R"))
} else {
    stop(paste("Requested covidm version", cm_version_, "but there are only versions 1 to 3."));
}
...
packageStartupMessage("FIX THIS - because of the late addition of the L compartment, some process sources are confusingly named.")
