...
# N.B the SGTF linelist downloaded from DSTL's sftp server on 3rd January 2022 
# no longer contains SGTF data from before August 2021. For fitting to Alpha, 
# use SGTF data builds from December 2021 (sgtf-2021-12-15.csv) or earlier

ll = fread("~/Documents/uk_covid_data_sensitive/phe/2022-01-10/Anonymised Combined Line List 20220110.csv")
sgtf = fread("~/Documents/uk_covid_data_sensitive/phe/2022-01-10/SGTF_linelist_20220110.csv")
ll[, specimen_date := dmy(specimen_date)]
sgtf[, specimen_date := ymd(specimen_date)]
ll$FINALID <- ll$finalid
d = merge(ll, sgtf, by = c("FINALID", "specimen_date"), all = TRUE)

#d[!is.na(age), agegroup := paste0(pmin((age %/% 10) * 10, 80), "-", pmin((age %/% 10) * 10, 80) + 9)]
# merging on 22nd June has created two age columns age.x and age.y, not sure why. age.x has fewer NA so using that for now
d[!is.na(age.x), agegroup := paste0(pmin((age.x %/% 10) * 10, 80), "-", pmin((age.x %/% 10) * 10, 80) + 9)]
...
fwrite(dd[pillar == "Pillar 2" & date >= "2020-10-01", .(date, nhs_name, sgtf, other)], 
       "./fitting_data/sgtf-2022-01-10-omicron-only.csv")
